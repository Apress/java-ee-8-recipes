...
import org.javaee8recipes.chapter07.Chapter;
...
public class ChapterDAO implements java.io.Serializable {
    ...
    /**
     * Returns the next ID in the CHAPTER_S sequence
     *
     * @return
     */
    public int getNextId() {
        String qry = "select chapter_s.currval as ID from dual";
        ...
        try (Connection conn = CreateConnection.getConnection();
                ...
                ResultSet rs = stmt.executeQuery(qry);) {
            while (rs.next()) {
                int chapter_id = rs.getInt("ID");
                returnId = chapter_id + 1;
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
        ...
        chapterDao.readClob();
        chapterDao.performCreate(100, 0, "Fake Title", "Fake Description");
        chapterDao.readClob();
    }
}
